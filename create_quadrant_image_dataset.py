# This code is to generate the image data set from IR_data nc file set.
...
def crop_hurricane_image_save(local_folder, hurricane_name, timestamp, nc_path, target_lat, target_lon, size_km, quadrant):
    ...
    if quadrant == 'NE':
        x_range = (center_x, center_x + offset)
        y_range = (center_y + offset, center_y)
    elif quadrant == 'SE':
        x_range = (center_x, center_x + offset)
        y_range = (center_y, center_y - offset)
    elif quadrant == 'SW':
        x_range = (center_x - offset, center_x)
        y_range = (center_y, center_y - offset)
    elif quadrant == 'NW':
        x_range = (center_x - offset, center_x)
        y_range = (center_y + offset, center_y)
    else:
        raise ValueError("Quadrant must be NE, SE, SW, or NW")
    
    x_min, x_max = x_range[0] / h, x_range[1] / h
    y_max, y_min = y_range[0] / h, y_range[1] / h
    ...
    npy_filename = f"{local_folder}/{hurricane_name}_{timestamp}_{quadrant}.npy"
    np.save(npy_filename, ir_final)
    print(f"Image shape: {ir_final.shape}, creating file: {npy_filename}")

    ds.close()

def create_hurricane_image_dataset(local_folder, root_dir, ir_data_dir, quadrant):
    # Folder Handle
...
if __name__ == "__main__":
    create_hurricane_image_dataset('NE_data', 'WSProfile', 'IR_data', 'NE')
    create_hurricane_image_dataset('SE_data', 'WSProfile', 'IR_data', 'SE')
    create_hurricane_image_dataset('SW_data', 'WSProfile', 'IR_data', 'SW')
    create_hurricane_image_dataset('NW_data', 'WSProfile', 'IR_data', 'NW')